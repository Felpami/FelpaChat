...
class felpa_client():
    ...
    def receive_msg(self, s, window_send):
        while True:
            try:
                message = s.recv(SIZE)
                msg = self.dec(message).split("[SEP]")
            ...
            if len(msg) == 3:
                ...
                window_send["TextBox"].print(": " + emoji.emojize(emoji_converter(msg[2])))
    ...
    def client(self):
        ...
        while True:
            ...
            if event == "Quit" or event == sg.WINDOW_CLOSED:
                self.quit = True
                try:
                    server.sendall(self.enc(self.username+"[SEP]"+self.color+"[SEP][QUIT]"))
                    #server.recv(SIZE)
                    time.sleep(0.05)
                    server.close()
                    window_send.close()
                ...
                if "[SEP]" in msg:
                    ...
                    try:
                        if count < 10:
                            server.sendall(self.enc(self.username+f" ({time.strftime('%H:%M:%S')})"+"[SEP]"+self.color+"[SEP]"+msg))
                            window_send["TextBox"].print(self.username+f" ({time.strftime('%H:%M:%S')})", text_color=self.color, end='')
