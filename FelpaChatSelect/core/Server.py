...
from core.emoji_felpa import emoji_converter
...
class felpa_server():
    ...
    def admin_command(self, command, window_send):
        if command.startswith("help"):
            ...
            if len(cmd) != 2:
                ...
                try:
                    self.conn_clients[position].close()
    ...
    def connect(self, conn, window_send):
        if self.dimension == 0:
            with conn:
        ...
        self.broadcast(f"{username} connected to FelpaChat[SEP]{color}",
                       self.conn_clients[1:index] + self.conn_clients[index + 1:])
        playsound.playsound("incoming.wav", False)
        conn.settimeout(None)

    def elaborate_msg(self, window_send):
        while True:
            if len(self.message_queue) > 0:
                msg_a = self.message_queue.pop(0).split("[SEP]")
                read = self.conn_queue.pop(0)
                username = msg_a[0]
                color = msg_a[1]
                msg = msg_a[2]

                # self.broadcast(msg, inputs)
                index = self.conn_clients.index(read)
                self.broadcast("[SEP]".join(msg_a), self.conn_clients[1:index] + self.conn_clients[index + 1:])
    ...
    def conn_loop(self, s, window_send):
        connect_t = threading.Thread(target=self.elaborate_msg, args=(window_send,), daemon=True)
        connect_t.start()
        while True:
            try:
                readable, _, exceptional = select.select(self.conn_clients, [], self.conn_clients)
            ...
            for read in readable:
                if read is s:
                    try:
                        ...
                        connect_t = threading.Thread(target=self.connect, args=(conn, window_send,), daemon=True)
                        connect_t.start()
                        connect_t.join()
                    except SocketError as e:
                        print("Error")
                    ...
                    try:
                        #msg_a = self.dec(read.recv(SIZE)).split("[SEP]")
                        msg = self.dec(read.recv(SIZE))
                        if msg.split("[SEP]")[2] != "[QUIT]":
                            self.message_queue.append(msg)
                            self.conn_queue.append(read)
                        else:
                            read.close()
                            username = self.username_a[self.conn_clients.index(read)]
                            self.username_a.remove(username)
                            color = self.user_color_a[self.conn_clients.index(read)]
                            self.conn_clients.remove(read)
                            self.user_color_a.remove(color)
                            self.color.append(color)
                            self.dimension += 1
                            window_send["TextBox"].print(f"{username} disconnected from FelpaChat", text_color='Orange')
                            self.broadcast(f"{username} disconnected from FelpaChat", self.conn_clients[1:])
                            self.user_list_update(window_send)
    ...
    def server(self):  # main shell function
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            conn_t = threading.Thread(target=self.conn_loop, args=(s, window_send,), daemon=True)
            conn_t.start()
            count = 0
            ...
            while (True):
                ...
                if event == "Quit" or event == sg.WINDOW_CLOSED:
                    self.quit = True
                    try:
                        #for connection in self.conn_clients[1:]:
                        #    connection.close()
                        s.close()
                        window_send.close()
                        break
                ...
                if msg.startswith("/"):
                    ...
                    if "[SEP]" in msg:
                        ...
                        #window_send["TextBox"].print("Cannot send message with keyword '[SEP]'.")
                        self.popup("Cannot send message with keyword '[QUIT]'.")
                    elif len(msg) > 300:
                        self.popup("Maximun message length is 300 character.")
                    elif len(msg) != 0:
                        if count < 10:
                            window_send["TextBox"].print(f"[{self.username}] ({time.strftime('%H:%M:%S')})", text_color="red", end='')
                            window_send["TextBox"].print(": " + emoji.emojize(emoji_converter(msg)))
                            self.broadcast(f"[{self.username}] ({time.strftime('%H:%M:%S')})[SEP]red[SEP]{msg}", self.conn_clients[1:])
                            count += 1
                        else:
